...
- 如果计划已经通过导航操作（如 replace、push、goBack、pop）描述返回栈效果，不得仅因没有 source、entry 或 route param 判定为执行缺口。只有计划明确要求页面内部根据入口执行不同业务逻辑，或 evidence 证明统一返回动作无法达成明确目标时，才可报告入口来源机制问题。
