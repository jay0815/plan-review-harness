...
- 新增未被输入证明的外部服务、流量治理、发布渠道、配置系统或业务损失等事实。
...
如果生产方把同一字段写入一个路径，而消费方从另一个不兼容路径读取：

- 当前方案存在确定的 wire contract 错误。
- 不能把“保持当前差异”作为有效决策选项。
- 可以选择统一到任一既定路径，但“必须统一”不需要用户重新裁决。
...
- 统一字段到哪个既定路径，通常是 L2。
- 需求已明确复用既有公共接口且不新增平行接口时，拆分两个公开接口属于超范围意见，应直接丢弃；只有需求未裁定 API surface、且两个方向都满足现有约束时，API 拆分才可能属于 L3。
- 未经输入支持的新平台、新存储或新基础设施不是待裁决的 L3 分歧，而是无 evidence 的超范围意见，应直接降权。
...
- 没有 evidence 的全新基础设施或存储系统。
- 没有 evidence 的全新消息平台。
- 把两个公开 API 当成唯一正确答案。
- 保证辅助信号零丢失、最终必达或因此阻塞业务。
- 无依据的兼容性 blocker。
...
- `consensus_issues[].required_plan_change` 是否为实质性计划修订目标；空字符串、无、无需新增修订或无需修改计划都表示该项不应进入 consensus。
- Fact Check 只支持弱事实、但否定阻塞性/直接后果/计划缺口时，候选是否只保留 `source_findings`，而不是生成 consensus 或 revision。
...
- “写入路径与读取路径不一致”是确定协议错误；最终位置可以讨论，但必须修复不需要讨论。
- 当需求明确要求复用既有接口且不新增平行接口时，API 拆分是超范围意见，不是 L3；只有需求本身未裁定 API surface 时才可能形成真实方向分歧。
- 未表态不等于支持，也不等于无分歧。
- 无 evidence 的新基础设施应直接降权，不能进入修订指令。
- 不得为旧版本编造新的幂等标识、业务字段组合去重或补丁方案。
- 回滚能力是修订目标，但具体开关系统、配置中心或下游下发机制不是默认实现。
