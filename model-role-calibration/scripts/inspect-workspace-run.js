...
function summarizeRole(roleDir) {
    ...
    const successfulReadFiles = unique(readAttempts.filter((attempt) => attempt.result?.is_error !== true).map((attempt) => attempt.file));
    const failedReadFiles = unique(readAttempts.filter((attempt) => attempt.result?.is_error === true).map((attempt) => attempt.file));
...
function main() {
    ...
    const summary = inspect((0, lib_js_1.requireArg)(args, 'run-dir'));
    if (args.json) {
