...
exports.DEFAULT_ROLE_ROUTES = exports.DEFAULT_ROLE_ROUTE_SOURCE = exports.DEFAULT_MODEL_FILES = exports.REQUIRED_ROLES = exports.WORKSPACE_ROLES = exports.MAX_EXECUTOR_RETRIES = exports.JSON_VALIDATOR_TOOL = exports.SYNTHESIS_ROLE = exports.FACT_CHECK_ROLE = exports.REVIEW_ROLES = exports.EXISTING_CODE_REFS_HEADING_PATTERN = void 0;
exports.assertWorkspaceRole = assertWorkspaceRole;
exports.expandHome = expandHome;
exports.resolveConfiguredPath = resolveConfiguredPath;
exports.validateSettingsFile = validateSettingsFile;
exports.validateClaudeBinary = validateClaudeBinary;
exports.loadWorkspaceReviewConfig = loadWorkspaceReviewConfig;
exports.loadWorkspaceReviewSettingsDirectory = loadWorkspaceReviewSettingsDirectory;
exports.loadWorkspaceReviewFromArgs = loadWorkspaceReviewFromArgs;
exports.configSummary = configSummary;
exports.validateProjectRoot = validateProjectRoot;
exports.buildRoleReadScope = buildRoleReadScope;
exports.buildFactCheckReadScope = buildFactCheckReadScope;
exports.copyScopedWorkspace = copyScopedWorkspace;
exports.compactPlanForReview = compactPlanForReview;
exports.createPlanReferenceManifest = createPlanReferenceManifest;
exports.collectPathCandidates = collectPathCandidates;
exports.existingCodeRefPaths = existingCodeRefPaths;
exports.lineRefSuffix = lineRefSuffix;
exports.workspaceReviewInput = workspaceReviewInput;
exports.buildWorkspacePrompt = buildWorkspacePrompt;
exports.workspaceSchemaForRole = workspaceSchemaForRole;
exports.buildClaudeWorkspaceArgs = buildClaudeWorkspaceArgs;
exports.runDirectory = runDirectory;
exports.executionLogPath = executionLogPath;
exports.appendExecutionLog = appendExecutionLog;
exports.updateState = updateState;
exports.redactedSettingsWarnings = redactedSettingsWarnings;
exports.withoutAnthropicApiKey = withoutAnthropicApiKey;
...
exports.REVIEW_ROLES = REVIEW_ROLES;
const FACT_CHECK_ROLE = 'fact_check';
exports.FACT_CHECK_ROLE = FACT_CHECK_ROLE;
const SYNTHESIS_ROLE = 'synthesis';
exports.SYNTHESIS_ROLE = SYNTHESIS_ROLE;
const JSON_VALIDATOR_TOOL = 'mcp__json_validator__validate_json_output';
exports.JSON_VALIDATOR_TOOL = JSON_VALIDATOR_TOOL;
const MAX_EXECUTOR_RETRIES = 3;
exports.MAX_EXECUTOR_RETRIES = MAX_EXECUTOR_RETRIES;
...
exports.DEFAULT_MODEL_FILES = DEFAULT_MODEL_FILES;
...
const DEFAULT_ROLE_ROUTE_FILE = path.join(lib_js_1.ROOT, 'default-role-routes.json');
const DEFAULT_ROLE_ROUTE_CONFIG = (0, lib_js_1.parseJsonFile)(DEFAULT_ROLE_ROUTE_FILE);
const DEFAULT_ROLE_ROUTES = Object.freeze({ ...DEFAULT_ROLE_ROUTE_CONFIG.routes });
exports.DEFAULT_ROLE_ROUTES = DEFAULT_ROLE_ROUTES;
...
exports.DEFAULT_ROLE_ROUTE_SOURCE = DEFAULT_ROLE_ROUTE_SOURCE;
...
exports.EXISTING_CODE_REFS_HEADING_PATTERN = EXISTING_CODE_REFS_HEADING_PATTERN;
...
function validateSettingsFile(model, modelConfig, configDir) {
    ...
    const settingsText = (0, lib_js_1.readText)(settingsFile);
    if (/"ANTHROPIC_API_KEY"\s*:/.test(settingsText)) {
        throw new Error(`Settings file for model "${model}" contains forbidden ANTHROPIC_API_KEY; ` + 'use ANTHROPIC_AUTH_TOKEN only');
    }
    let settings;
    try {
    ...
    const requiredEnv = Array.isArray(modelConfig.required_env) ? modelConfig.required_env : ['ANTHROPIC_BASE_URL'];
...
function normalizeWorkspaceReviewConfig(raw, source, options = {}) {
    ...
    const normalized = {
        ...
        claude_version: null,
        workspace_runs_dir: resolveConfiguredPath(raw.workspace_runs_dir || path.join(lib_js_1.ROOT, 'workspace-runs'), configDir),
        models,
        roles,
        execution: {
            ...
            read_scope_max_files: positiveInteger(execution.read_scope_max_files || DEFAULT_READ_SCOPE_MAX_FILES, 'execution.read_scope_max_files'),
        },
    };
...
function loadWorkspaceReviewConfig(configFile, options = {}) {
    ...
    let raw;
    try {
        raw = (0, lib_js_1.parseJsonFile)(absoluteConfigFile);
    }
    catch (error) {
...
function loadWorkspaceReviewSettingsDirectory(settingsDir, options = {}) {
    ...
    const models = Object.fromEntries(Object.entries(DEFAULT_MODEL_FILES).map(([model, filename]) => [
        model,
        {
    ...
    return normalizeWorkspaceReviewConfig({
        ...
        workspace_runs_dir: options.workspaceRunsDir || path.join(lib_js_1.ROOT, 'workspace-runs'),
        models,
    ...
    while (stack.length && matches.length <= 1) {
        ...
        for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
            ...
            if (entry.isDirectory()) {
                ...
                if (suffixVariants.some((suffix) => relative === suffix ||
...
function collectPathCandidates(text) {
    ...
    const patterns = [
        ...
        new RegExp(`(?:^|[\\s"'([])((?:\\.{1,2}/)?(?:[A-Za-z0-9_.@+-]+/)+[A-Za-z0-9_.@+-]+(?:\\.[A-Za-z0-9]+)?${lineRef})`, 'gm'),
...
function normalizeProposedArtifacts(artifacts = []) {
    return (Array.isArray(artifacts) ? artifacts : [])
        ...
        return (relative.startsWith(`${PROPOSED_CODE_DIR}/`) && !relative.includes('\0') && !relative.split('/').includes('..'));
    });
}
...
function createPlanReferenceManifest(projectRoot, plan, proposedArtifacts = []) {
    ...
    return {
        ...
        existing_code_refs: [...existingRefsByKey.values()].sort((a, b) => `${a.path}:${a.line_ref || ''}`.localeCompare(`${b.path}:${b.line_ref || ''}`)),
        existing_code_ref_dirs: [...existingRefDirsByKey.values()].sort((a, b) => `${a.path}:${a.line_ref || ''}`.localeCompare(`${b.path}:${b.line_ref || ''}`)),
...
function buildReadScopeFromText(role, projectRoot, text, options = {}) {
    ...
    const artifactsByPath = new Map(normalizeProposedArtifacts(options.proposedArtifacts).map((artifact) => [artifact.relative_path, artifact]));
...
function buildFactCheckReadScope(projectRoot, reviewerOutputs, options = {}) {
    const text = Object.values(reviewerOutputs || {})
        .flatMap((output) => (Array.isArray(output?.issues) ? output.issues : [])
...
function copyScopedWorkspace(projectRoot, readScope, workspaceParent) {
    ...
    const copied = [];
    const proposedByPath = new Map((readScope.proposed_artifacts || []).map((artifact) => [artifact.relative_path, artifact]));
    ...
    return [
        ...
        proposedArtifacts.length
            ? [
                ...
                ...proposedArtifacts.map((artifact) => `- ${artifact.relative_path}:1-${artifact.line_count || '?'} ` +
...
function extractCodeSignals(code) {
    ...
    for (const line of lines) {
        ...
        const declaration = trimmed.match(/^(?:export\s+)?(?:async\s+)?(?:function|class|interface|type|const|let|var)\s+([A-Za-z0-9_$]+)/);
        if (declaration) {
        ...
        if (/\b(await|return|throw|expect|send|goto|click|write|close|create|clear|emit)\b/.test(trimmed) &&
            trimmed.length <= 160) {
...
function compactPlanForReview(plan, options = {}) {
    ...
    const compacted = originalPlan.replace(/```([^\n`]*)\n([\s\S]*?)```/g, (match, rawLanguage, code) => {
        ...
        const shouldPreserve = PRESERVE_CODE_BLOCK_LANGS.has(language) || (lineCount < lineThreshold && code.length < charThreshold);
        if (shouldPreserve) {
            preservedBlocks += 1;
            return match;
        }
    ...
    const planBloatWarning = codeBlocks >= 6 ||
        compactedBlocks >= 3 ||
...
function buildWorkspacePrompt(role, projectRoot, plan, context = '', reviewerOutputs = null, factCheckOutput = null, readBoundary = null) {
    assertWorkspaceRole(role);
    const templateFile = path.join(lib_js_1.ROOT, 'prompts', `probe-${role}.md`);
    let template = (0, lib_js_1.readText)(templateFile);
    if (role === 'rebuttal') {
        template = template
            .replace('下面这个方案已经被专家认可。请你仍然独立审查它是否存在问题。', '请在不受其他 Reviewer 结论影响的前提下，独立审查下面的计划是否存在问题。')
...
function workspaceSchemaForRole(role) {
    assertWorkspaceRole(role);
    if (role === FACT_CHECK_ROLE) {
        return path.join(lib_js_1.ROOT, 'schemas', 'fact-check-output.schema.json');
    }
    return (0, lib_js_1.schemaForProbe)(role);
}
function buildClaudeWorkspaceArgs(config, model, role, projectRoot, options = {}) {
    ...
    const args = [
        ...
        JSON.stringify({
            mcpServers: {
                json_validator: {
                    ...
                    args: [path.join(lib_js_1.ROOT, 'scripts', 'json-validator-mcp.js')],
                    env: {
        ...
        'stream-json',
        '--json-schema',
        JSON.stringify((0, lib_js_1.parseJsonFile)(schemaFile)),
        '--max-turns',
...
function appendExecutionLog(runDir, event, details = {}) {
    const fields = Object.entries(details)
        ...
        .join(' ');
    fs.appendFileSync(executionLogPath(runDir), `[${new Date().toISOString()}] ${event}${fields ? ` ${fields}` : ''}\n`, 'utf8');
}
function updateState(runDir, patch) {
    const stateFile = path.join(runDir, 'state.json');
    let current = {};
    if (fs.existsSync(stateFile)) {
        current = (0, lib_js_1.parseJsonFile)(stateFile);
    }
    const next = {
        ...current,
        ...patch,
    ...
    (0, lib_js_1.writeGenerated)(stateFile, JSON.stringify(next, null, 2) + '\n');
    return next;
}
