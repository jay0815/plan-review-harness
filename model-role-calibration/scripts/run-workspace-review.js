...
exports.runRole = runRole;
exports.runWithConcurrency = runWithConcurrency;
exports.summarizeReviewOutcome = summarizeReviewOutcome;
exports.reviewerStageError = reviewerStageError;
exports.runSynthesis = runSynthesis;
exports.retryWorkspaceReviewStage = retryWorkspaceReviewStage;
exports.completedReviewerResult = completedReviewerResult;
exports.normalizedRetryCounts = normalizedRetryCounts;
exports.assertRetryAvailable = assertRetryAvailable;
exports.loadCompletedReviewerResults = loadCompletedReviewerResults;
exports.loadCompletedFactCheckResult = loadCompletedFactCheckResult;
exports.writeWorkspaceReport = writeWorkspaceReport;
exports.validateWorkspaceOutput = validateWorkspaceOutput;
exports.validateSynthesisSemantics = validateSynthesisSemantics;
...
function writeJson(file, value) {
    (0, lib_js_1.writeGenerated)(file, JSON.stringify(value, null, 2) + '\n');
}
...
function reviewerSeverityByIssueId(reviewerOutputs = {}) {
    ...
    for (const [source, reviewerOutput] of Object.entries(reviewerOutputs || {})) {
        ...
        issues.forEach((issue, index) => {
            const issueId = `${(0, lib_js_1.slug)(source)}-${String(index + 1).padStart(3, '0')}`;
            if (issue?.severity) {
...
function validateSynthesisSemantics(output, factCheckOutput, reviewerOutputs = {}) {
    ...
    for (const finding of findings) {
        ...
        if (finding.source_issue_id) {
            if (byIssueId.has(finding.source_issue_id)) {
                throw new Error(`Synthesis semantic validation failed: duplicate source_issue_id ${finding.source_issue_id} on findings ${byIssueId.get(finding.source_issue_id).id} and ${finding.id}`);
            }
            byIssueId.set(finding.source_issue_id, finding);
        }
    }
    const checkedIssues = Array.isArray(factCheckOutput?.checked_issues) ? factCheckOutput.checked_issues : [];
    ...
    for (const checked of checkedIssues) {
        ...
        if (finding.source !== checked.source) {
            throw new Error(`Synthesis semantic validation failed: ${finding.id} source ${finding.source} != ${checked.source} for issue_id ${checked.issue_id}`);
        }
        if (finding.source_title !== checked.issue_title) {
            throw new Error(`Synthesis semantic validation failed: ${finding.id} source_title ${finding.source_title} != ${checked.issue_title} for issue_id ${checked.issue_id}`);
        }
        if (finding.fact_check_status !== checked.status) {
            throw new Error(`Synthesis semantic validation failed: ${finding.id} fact_check_status ` +
                `${finding.fact_check_status} != ${checked.status}`);
        }
        if (finding.scope_status !== checked.scope_status) {
            throw new Error(`Synthesis semantic validation failed: ${finding.id} scope_status ` +
        ...
        if (!requiredDisposition &&
            ...
            throw new Error(`Synthesis semantic validation failed: ${finding.id} verified finding cannot use disposition ${finding.disposition}`);
        }
    }
    for (const finding of findings) {
        if (finding.source_issue_id && !checkedIds.has(finding.source_issue_id)) {
            throw new Error(`Synthesis semantic validation failed: source finding ${finding.id} has no matching fact_check entry for issue_id ${finding.source_issue_id}`);
        }
    }
    const referencedIds = (items, field = 'source_finding_ids') => (Array.isArray(items) ? items : []).flatMap((item) => item?.[field] || []);
    const activeIds = [
    ...
    for (const id of referencedIds(output.likely_false_positives)) {
        const finding = byId.get(id);
        if (!excludedDispositions.has(finding.disposition)) {
            throw new Error(`Synthesis semantic validation failed: likely_false_positives cannot reference retained finding ${id}`);
        }
    }
    ...
    for (const node of processNodes) {
        for (const title of node.related_issue_titles || []) {
            if (!issueTitles.has(title)) {
                throw new Error(`Synthesis semantic validation failed: process_map node ${node.id} references unknown issue title ${title}`);
            }
        }
    }
    for (const item of [...(output.consensus_issues || []), ...(output.disagreements || [])]) {
        for (const nodeId of item.affected_nodes || []) {
            if (!nodeIds.has(nodeId)) {
                throw new Error(`Synthesis semantic validation failed: ${item.title} references unknown process_map node ${nodeId}`);
            }
        }
    }
    for (const item of output.consensus_issues || []) {
        ...
        for (const source of mergedFrom) {
            if (!sources.has(source)) {
                throw new Error(`Synthesis semantic validation failed: ${item.title} merged_from includes source without finding ${source}`);
            }
        }
    }
    ...
    for (const instruction of output.revision_instructions || []) {
        for (const id of instruction.source_finding_ids || []) {
            ...
            if (!['verified', 'partially_verified'].includes(finding.fact_check_status)) {
                throw new Error(`Synthesis semantic validation failed: revision instruction references non-verified finding ${id}`);
            }
            ...
            const linkedConsensus = (output.consensus_issues || []).filter((item) => (item.source_finding_ids || []).includes(id));
            for (const consensus of linkedConsensus) {
                ...
                if (consensusRank > reviewerRank) {
                    throw new Error(`Synthesis semantic validation failed: partially_verified finding ${id} severity ${consensus.severity} exceeds reviewer severity ${reviewerSeverity}`);
                }
            }
        }
    }
    for (const disagreement of output.disagreements || []) {
        ...
        if (disagreement.needs_human_decision !== shouldNeedHuman) {
            throw new Error(`Synthesis semantic validation failed: ${disagreement.title} needs_human_decision ` +
...
function validateExecutionSemantics(output) {
    ...
    for (const issue of output?.issues || []) {
        ...
        if (!expected.some((boundary) => coveredBoundaries.has(boundary))) {
            throw new Error(`Execution semantic validation failed: issue "${issue.title}" type ${issue.type} ` +
                `is not covered by coverage_declaration`);
        }
    }
}
function validateWorkspaceOutput(role, output, context = {}) {
    const validation = (0, json_validator_mcp_js_1.validateJsonText)(JSON.stringify(output), (0, lib_js_1.parseJsonFile)((0, workspace_review_lib_js_1.workspaceSchemaForRole)(role)));
    if (!validation.valid) {
...
function materializeProposedArtifacts(runDir, artifacts = []) {
    return artifacts.map((artifact) => {
        ...
        (0, lib_js_1.writeGenerated)(sourceFile, artifact.content || '');
        return {
...
function summarizeFactCheckOutput(output) {
    ...
    const challenged = ['partially_verified', 'unsupported', 'contradicted', 'unverifiable'].reduce((sum, key) => sum + (statusCounts[key] || 0), 0);
...
function summarizeReviewOutcome(reviewerResults, factCheck, synthesis, infraErrors, authoringLint = null) {
    const reviewerIssueCount = reviewerResults.reduce((sum, item) => sum + (Array.isArray(item.output?.issues) ? item.output.issues.length : 0), 0);
...
function readJsonIfExists(file) {
    ...
    return (0, lib_js_1.parseJsonFile)(file);
}
function completedReviewerResult(runDir, role) {
    ...
    const metadata = (0, lib_js_1.parseJsonFile)(metadataFile);
    ...
    return {
        ...
        output: (0, lib_js_1.parseJsonFile)(outputFile),
...
function loadCompletedFactCheckResult(runDir, retryStage = 'synthesis') {
    const roleDir = path.join(runDir, 'roles', workspace_review_lib_js_1.FACT_CHECK_ROLE);
    const outputFile = path.join(roleDir, 'output.json');
    ...
    return {
        role: workspace_review_lib_js_1.FACT_CHECK_ROLE,
        model: metadata.model,
        output: (0, lib_js_1.parseJsonFile)(outputFile),
        output_file: path.relative(runDir, outputFile),
        summary: (0, lib_js_1.parseJsonFile)(summaryFile),
...
function loadRequestForRun(config, runDir) {
    ...
    const request = (0, lib_js_1.parseJsonFile)(requestFile);
    request.project_root = (0, workspace_review_lib_js_1.validateProjectRoot)(request.project_root);
    ...
    if (!request.authoring_lint) {
        request.authoring_lint = (0, plan_authoring_lint_js_1.lintPlan)({
            plan: request.plan,
            projectRoot: request.project_root,
        });
        writeJson(path.join(runDir, 'plan-authoring-lint.json'), request.authoring_lint);
    }
    const roles = Array.isArray(request.roles) && request.roles.length ? request.roles : workspace_review_lib_js_1.REVIEW_ROLES;
    for (const role of roles) {
        if (!workspace_review_lib_js_1.REVIEW_ROLES.includes(role)) {
...
function reviewerInfraError(role, model, error, runDir) {
    return {
        role,
        model,
        type: /invalid output|valid JSON|Probe mismatch/i.test(error.message) ? 'invalid_output' : 'agent_failed',
...
function prepareReadBoundary(config, request, runDir, role, readScope) {
    ...
    const boundary = {
        ...readScope,
        ...(0, workspace_review_lib_js_1.copyScopedWorkspace)(request.project_root, readScope, workspaceParent),
    };
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'read_scope_prepared', {
        role,
        mode: boundary.mode,
...
async function runRole(config, request, role, runDir) {
    ...
    const readScope = (0, workspace_review_lib_js_1.buildRoleReadScope)(role, request.project_root, request.review_plan || request.plan, {
    ...
    const prompt = (0, workspace_review_lib_js_1.buildWorkspacePrompt)(role, readBoundary.promptRoot, request.review_plan || request.plan, request.context || '', null, null, readBoundary.boundary);
    const promptFile = path.join(roleDir, 'prompt.md');
    const validatorLogFile = path.join(roleDir, 'validator.log');
    (0, lib_js_1.writeGenerated)(promptFile, prompt);
    const args = (0, workspace_review_lib_js_1.buildClaudeWorkspaceArgs)(config, model, role, readBoundary.claudeRoot, {
        validatorLogFile,
    });
    const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'plan-review-role-'));
    const startedAt = new Date().toISOString();
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'agent_started', {
        role,
        model,
    });
    let child;
    try {
        child = await (0, run_model_js_1.runCommand)(config.claude_bin, args, {
            cwd: workDir,
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'agent_failed', {
            role,
            model,
        ...
        writeRoleMetadata(runDir, roleDir, {
            ...
            validator_tool: workspace_review_lib_js_1.JSON_VALIDATOR_TOOL,
            validator_log_file: path.relative(runDir, validatorLogFile),
            schema_file: path.relative(lib_js_1.ROOT, (0, workspace_review_lib_js_1.workspaceSchemaForRole)(role)),
            project_root: request.project_root,
            read_boundary: {
    ...
    (0, lib_js_1.writeGenerated)(path.join(roleDir, 'stdout.jsonl'), child.stdout || '');
    (0, lib_js_1.writeGenerated)(path.join(roleDir, 'stderr.log'), child.stderr || '');
    const metadata = {
        role,
        model,
        started_at: startedAt,
    ...
    let parsed;
    try {
        parsed = (0, run_model_js_1.parseAssistantOutput)(child.stdout, role);
        validateWorkspaceOutput(role, parsed.output);
    }
    catch (error) {
        (0, lib_js_1.writeGenerated)(path.join(roleDir, 'output.invalid.txt'), extractFinalOutputText(child.stdout));
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'agent_invalid_output', {
            role,
            model,
            elapsed_ms: Date.now() - startedMs,
        });
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'agent_completed', {
        role,
        model,
        elapsed_ms: Date.now() - startedMs,
    });
...
async function runFactCheck(config, request, reviewerResults, runDir) {
    const role = workspace_review_lib_js_1.FACT_CHECK_ROLE;
    const model = config.roles[role];
    const startedMs = Date.now();
    const roleDir = path.join(runDir, 'roles', role);
    fs.mkdirSync(roleDir, { recursive: true });
    const reviewerOutputs = Object.fromEntries(reviewerResults.map((item) => [SOURCE_NAME_BY_ROLE[item.role], item.output]));
    const readScope = (0, workspace_review_lib_js_1.buildFactCheckReadScope)(request.project_root, reviewerOutputs, {
        maxFiles: config.execution.read_scope_max_files,
        proposedArtifacts: request.proposed_artifacts || [],
    ...
    const readBoundary = prepareReadBoundary(config, request, runDir, role, readScope);
    writeJson(path.join(roleDir, 'read-scope.json'), readBoundary.boundary);
    const prompt = (0, workspace_review_lib_js_1.buildWorkspacePrompt)(role, readBoundary.promptRoot, request.review_plan || request.plan, request.context || '', reviewerOutputs, null, readBoundary.boundary);
    const promptFile = path.join(roleDir, 'prompt.md');
    const validatorLogFile = path.join(roleDir, 'validator.log');
    (0, lib_js_1.writeGenerated)(promptFile, prompt);
    const args = (0, workspace_review_lib_js_1.buildClaudeWorkspaceArgs)(config, model, role, readBoundary.claudeRoot, {
    ...
    const startedAt = new Date().toISOString();
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'fact_check_started', {
        role,
        model,
    ...
    try {
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'fact_check_failed', {
            role,
            model,
            elapsed_ms: Date.now() - startedMs,
        });
    ...
    try {
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'fact_check_invalid_output', {
            role,
            model,
            elapsed_ms: Date.now() - startedMs,
        });
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'fact_check_summary', factCheckSummary);
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'fact_check_completed', {
        role,
        model,
        elapsed_ms: Date.now() - startedMs,
    });
...
function reviewerStageError(reviewerResults, infraErrors) {
    ...
    const detail = infraErrors
        ...
        .join('; ');
    const error = new Error(`Reviewer stage failed before fact_check; retry reviewers first: ${detail}`);
...
async function runSynthesis(config, request, reviewerResults, factCheckResult, runDir) {
    ...
    fs.mkdirSync(roleDir, { recursive: true });
    const reviewerOutputs = Object.fromEntries(reviewerResults.map((item) => [SOURCE_NAME_BY_ROLE[item.role], item.output]));
    const prompt = (0, workspace_review_lib_js_1.buildWorkspacePrompt)(role, request.project_root, request.review_plan || request.plan, request.context || '', reviewerOutputs, factCheckResult.output);
    const promptFile = path.join(roleDir, 'prompt.md');
    const validatorLogFile = path.join(roleDir, 'validator.log');
    (0, lib_js_1.writeGenerated)(promptFile, prompt);
    const args = (0, workspace_review_lib_js_1.buildClaudeWorkspaceArgs)(config, model, role, request.project_root, {
        tools: '',
    ...
    const startedAt = new Date().toISOString();
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'synthesis_started', {
        role,
        model,
        reviewer_count: reviewerResults.length,
    });
    ...
    try {
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'synthesis_failed', {
            role,
            model,
            elapsed_ms: Date.now() - startedMs,
        });
    ...
    try {
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'synthesis_invalid_output', {
            role,
            model,
            elapsed_ms: Date.now() - startedMs,
        });
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'synthesis_completed', {
        role,
        model,
        elapsed_ms: Date.now() - startedMs,
    });
...
function archiveRoleAttempt(runDir, role) {
    ...
    (0, workspace_review_manifest_js_1.archiveResolvedExecutionAttempt)(runDir, role, archivedRoleDir);
    return archivedRoleDir;
}
function normalizedRetryCounts(state) {
    const counts = {};
    for (const role of [...workspace_review_lib_js_1.REVIEW_ROLES, workspace_review_lib_js_1.FACT_CHECK_ROLE, 'synthesis']) {
...
function assertRetryAvailable(retryCounts, executors) {
    const exhausted = [...new Set(executors)].filter((executor) => retryCounts[executor] >= workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES);
    if (exhausted.length) {
        throw new Error(`Retry limit reached (${workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES}) for executor(s): ${exhausted.join(', ')}`);
    }
}
function consumeExecutorRetries(runDir, retryCounts, executors) {
    ...
    (0, workspace_review_lib_js_1.updateState)(runDir, {
        retry_counts: retryCounts,
    });
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'executor_retries_consumed', {
        executors: [...new Set(executors)],
        retry_counts: retryCounts,
        retry_limit: workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES,
    });
}
function updateRunRetryManifest(runDir, stage, retryRoles) {
    (0, workspace_review_manifest_js_1.updateRunManifest)(runDir, {
        retry_stage: stage,
...
async function retryWorkspaceReviewStage(config, runDir, stage, options = {}) {
    if (!['reviewers', workspace_review_lib_js_1.FACT_CHECK_ROLE, 'synthesis'].includes(stage)) {
        throw new Error(`Unsupported retry stage: ${stage}. Expected reviewers, ${workspace_review_lib_js_1.FACT_CHECK_ROLE}, or synthesis.`);
    }
    ...
    (0, workspace_review_manifest_js_1.requireRunManifest)(absoluteRunDir);
    ...
    const completedReviewers = new Map(roles
    ...
    const plannedExecutors = stage === 'reviewers'
        ? [...retryRoles, workspace_review_lib_js_1.FACT_CHECK_ROLE, 'synthesis']
        : stage === workspace_review_lib_js_1.FACT_CHECK_ROLE
            ? [workspace_review_lib_js_1.FACT_CHECK_ROLE, 'synthesis']
            : ['synthesis'];
    ...
    else if (stage === workspace_review_lib_js_1.FACT_CHECK_ROLE) {
        archivedAttempts[workspace_review_lib_js_1.FACT_CHECK_ROLE] = archiveRoleAttempt(absoluteRunDir, workspace_review_lib_js_1.FACT_CHECK_ROLE);
        archivedAttempts.synthesis = archiveRoleAttempt(absoluteRunDir, 'synthesis');
    }
    else {
        archivedAttempts.synthesis = archiveRoleAttempt(absoluteRunDir, 'synthesis');
    }
    (0, workspace_review_lib_js_1.updateState)(absoluteRunDir, {
        status: 'running',
        pid: process.pid,
        retry_stage: stage,
        retry_started_at: new Date().toISOString(),
    ...
    (0, workspace_review_manifest_js_1.markManifestRunning)(absoluteRunDir, request);
    updateRunRetryManifest(absoluteRunDir, stage, retryRoles);
    (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'stage_retry_started', {
        stage,
        retry_roles: retryRoles,
    ...
    try {
        ...
        if (stage === 'reviewers') {
            ...
            if (failures.length) {
                throw new Error(`Reviewer retry failed: ${failures.map((item) => `${item.role}: ${item.error.message}`).join('; ')}`);
            }
            ...
            archivedAttempts[workspace_review_lib_js_1.FACT_CHECK_ROLE] = archiveRoleAttempt(absoluteRunDir, workspace_review_lib_js_1.FACT_CHECK_ROLE);
            archivedAttempts.synthesis = archiveRoleAttempt(absoluteRunDir, 'synthesis');
            (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'stage_retry_downstream_invalidated', {
                stage,
                archived_attempts: {
                    fact_check: archivedAttempts[workspace_review_lib_js_1.FACT_CHECK_ROLE],
        ...
        if (stage === 'synthesis') {
            ...
            if (stage === workspace_review_lib_js_1.FACT_CHECK_ROLE) {
                consumeExecutorRetries(absoluteRunDir, retryCounts, [workspace_review_lib_js_1.FACT_CHECK_ROLE]);
            }
        ...
        (0, workspace_review_manifest_js_1.markManifestFinished)(absoluteRunDir, 'completed', {
            retry_stage: null,
            infra_errors: [],
        });
        (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'stage_retry_completed', {
            stage,
        });
        (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'run_completed', {
            run_id: request.run_id,
            reviewer_count: reviewerResults.length,
            infra_error_count: 0,
        });
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'stage_retry_failed', {
            stage,
        });
        (0, workspace_review_lib_js_1.appendExecutionLog)(absoluteRunDir, 'run_failed', {
            run_id: request.run_id,
        });
        (0, workspace_review_lib_js_1.updateState)(absoluteRunDir, {
            status: 'failed',
        ...
        (0, workspace_review_manifest_js_1.markManifestFinished)(absoluteRunDir, 'failed', {
            retry_stage: null,
            error: error.stack || error.message,
        });
        throw error;
...
async function main() {
    ...
    const config = (0, workspace_review_lib_js_1.loadWorkspaceReviewFromArgs)(args);
    const { request, roles } = loadRequestForRun(config, runDir);
    (0, workspace_review_manifest_js_1.requireRunManifest)(runDir);
    const reviewPlan = config.execution.compact_plan
        ? (0, workspace_review_lib_js_1.compactPlanForReview)(request.plan)
        : {
            text: request.plan,
            stats: {
                original_chars: String(request.plan).length,
    ...
    request.review_plan_refs = (0, workspace_review_lib_js_1.createPlanReferenceManifest)(request.project_root, request.plan, request.proposed_artifacts);
    (0, lib_js_1.writeGenerated)(path.join(runDir, 'review-plan.md'), request.review_plan);
    ...
    (0, workspace_review_manifest_js_1.markManifestRunning)(runDir, request);
    (0, workspace_review_lib_js_1.updateState)(runDir, {
        status: 'running',
        pid: process.pid,
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'run_started', {
        run_id: request.run_id,
        pid: process.pid,
        roles,
        max_concurrency: config.execution.max_concurrency,
    });
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'plan_compacted', request.plan_compaction);
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'plan_authoring_linted', {
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'proposed_artifacts_prepared', {
    ...
    try {
        ...
        if (reviewerFailure) {
            (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'reviewers_failed', {
        ...
        (0, workspace_review_manifest_js_1.markManifestFinished)(runDir, 'completed', {
            infra_errors: infraErrors,
        });
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'run_completed', {
            run_id: request.run_id,
            reviewer_count: reviewerResults.length,
            infra_error_count: infraErrors.length,
        });
        ...
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'run_failed', {
            run_id: request.run_id,
            infra_error_count: infraErrors.length,
        });
        (0, workspace_review_lib_js_1.updateState)(runDir, {
        ...
        (0, workspace_review_manifest_js_1.markManifestFinished)(runDir, 'failed', {
            error: error.stack || error.message,
            infra_errors: infraErrors,
        });
        throw error;
