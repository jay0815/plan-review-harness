...
exports.resolveWrapperCommand = resolveWrapperCommand;
...
function parseJsonEnvelope(stdout) {
    ...
    const context = position !== null && Number.isInteger(position)
...
function resolveWrapperCommand(shell, model, timeoutMs, maxBuffer) {
    ...
    const child = (0, node_child_process_1.spawnSync)(shell, ['-lic', resolver], {
        encoding: null,
        timeout: timeoutMs,
        killSignal: 'SIGKILL',
        maxBuffer,
...
function nextAttempt(paths) {
    (0, lib_js_1.ensureDir)(paths.attemptsDir);
    const attempts = fs
...
function runCommand(command, args, options) {
    return new Promise((resolve) => {
        ...
        const heartbeat = setInterval(() => {
            ...
            logProgress(`still running after ${durationLabel(Date.now() - startedAt)} ` +
        ...
        child.on('close', (code, signal) => {
            ...
            const validatorStatus = validatorLogSummary(options.validatorLogFile);
            logProgress(`process finished after ${durationLabel(Date.now() - startedAt)} ` +
...
function writeCompletedArtifacts(paths, attempt, metadata, envelope, output) {
    ...
    metadata.error = null;
    (0, lib_js_1.writeFileNew)(attempt.rawJsonFile, JSON.stringify(envelope, null, 2) + '\n');
    (0, lib_js_1.writeFileNew)(attempt.resultFile, JSON.stringify(output, null, 2) + '\n');
    (0, lib_js_1.writeFileNew)(attempt.metadataFile, JSON.stringify(metadata, null, 2) + '\n');
    (0, lib_js_1.writeGenerated)(paths.rawFile, JSON.stringify(envelope, null, 2) + '\n');
    (0, lib_js_1.writeGenerated)(paths.metadataFile, JSON.stringify(metadata, null, 2) + '\n');
    (0, lib_js_1.writeGenerated)(paths.resultFile, JSON.stringify(output, null, 2) + '\n');
}
function reparseAttempt(paths, sourceLabel, newAttempt, metadataBase, probe) {
    ...
    const stdout = (0, lib_js_1.readText)(sourceRawFile);
    const parsed = parseAssistantOutput(stdout, probe);
    let sourceMetadata = {};
    if (fs.existsSync(source.metadataFile)) {
        sourceMetadata = (0, lib_js_1.parseJsonFile)(source.metadataFile);
    }
    const metadata = {
        ...metadataBase,
...
async function main() {
    ...
    const model = (0, lib_js_1.requireArg)(args, 'model').toLowerCase();
    ...
    const paths = (0, lib_js_1.agentOutputPaths)(run, caseId, model, probe);
    ...
    (0, lib_js_1.ensureDir)(paths.outputDir);
    const schemaFile = (0, lib_js_1.schemaForProbe)(probe);
    const schema = (0, lib_js_1.parseJsonFile)(schemaFile);
    const executionConfig = config.agent_execution;
    const timeoutMs = positiveInteger(args['timeout-ms'] && args['timeout-ms'] !== true ? args['timeout-ms'] : executionConfig.timeout_ms, '--timeout-ms');
    const aliasTimeoutMs = positiveInteger(executionConfig.alias_resolution_timeout_ms, 'agent_execution.alias_resolution_timeout_ms');
    const maxBuffer = positiveInteger(executionConfig.max_buffer_bytes, 'agent_execution.max_buffer_bytes');
    const reparseAttemptLabel = args['reparse-attempt'] && args['reparse-attempt'] !== true ? String(args['reparse-attempt']) : null;
    if (reparseAttemptLabel) {
        ...
        const metadataBase = {
            ...
            prompt_file: path.relative(lib_js_1.ROOT, promptFile),
    ...
    logProgress(`run=${run} case=${caseId} model=${model} probe=${probe} attempt=${attempt.label}`);
    logProgress(`prompt=${path.relative(lib_js_1.ROOT, promptFile)} schema=${path.relative(lib_js_1.ROOT, schemaFile)}`);
    ...
    try {
        ...
        child = await runCommand(wrapper.command, cliArgs, {
            cwd: workDir,
            input: (0, lib_js_1.readText)(promptFile),
            timeoutMs,
            killSignal: 'SIGKILL',
            maxBuffer,
            validatorLogFile,
    ...
    const metadata = {
        ...
        validator_log_file: validatorLogFile ? path.relative(lib_js_1.ROOT, validatorLogFile) : null,
        stderr: child.stderr || '',
    ...
    if (child.error || child.status !== 0) {
        if (child.stdout) {
            (0, lib_js_1.writeFileNew)(attempt.rawTextFile, child.stdout);
