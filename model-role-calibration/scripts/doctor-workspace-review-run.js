...
const {
  resolveRunDir,
  verifyRun
} = require("./verify-workspace-review-run");
...
function summarizeRefs(runDir) {
  ...
  return {
    ...
    refs_scoped_to_existing_code_refs_section:
      refs.format_status?.refs_scoped_to_existing_code_refs_section ?? null
  };
}
  ...
  if (!verification.valid) {
    addAction(actions, {
      ...
      command: shellCommand([
        "node",
        path.join(__dirname, "verify-workspace-review-run.js"),
        "--run-dir",
        runDir
      ])
    });
