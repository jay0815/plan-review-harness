...
exports.doctorWorkspaceReviewRun = doctorWorkspaceReviewRun;
...
function buildActions({ runDir, verification, lint, refs, synthesis }) {
    ...
    if (refs.present && refs.refs_scoped_to_existing_code_refs_section !== true) {
        addAction(actions, {
            ...
            reason: 'review-plan-refs 未确认引用只来自 Existing Code Refs / 现有代码映射章节；需要检查 plan ref manifest 生成。',
            file: refs.file,
        });
    }
...
function doctorWorkspaceReviewRun(runDir) {
    const absoluteRunDir = path.resolve(runDir);
    const verification = (0, verify_workspace_review_run_js_1.verifyRun)(absoluteRunDir);
...
function printText(result) {
    ...
    console.log(`Verify: ${result.verification.valid === null ? 'NOT_READY' : result.verification.valid ? 'PASS' : 'FAIL'} (${result.verification.counts.pass} pass, ${result.verification.counts.warn} warn, ${result.verification.counts.fail} fail, ${result.verification.counts.pending} pending)`);
    ...
    console.log(`plan-authoring-lint: ${result.plan_authoring_lint.present ? `${result.plan_authoring_lint.error_count} error(s), ${result.plan_authoring_lint.warning_count} warning(s), existing refs ${result.plan_authoring_lint.existing_code_ref_count}` : 'missing'}`);
    console.log(`review-plan-refs: ${result.review_plan_refs.present ? `${result.review_plan_refs.existing_file_ref_count} file ref(s), ${result.review_plan_refs.existing_dir_ref_count} dir ref(s), ${result.review_plan_refs.skipped_ref_count} skipped` : 'missing'}`);
    console.log(`Fact Check: ${result.fact_check.present ? `${result.fact_check.read_count} read(s), strictness ${result.fact_check.strictness_signal || '-'}` : 'missing'}`);
    console.log(`Synthesis: ${result.synthesis.present ? `${result.synthesis.consensus_issue_count} consensus, ${result.synthesis.disagreement_count} disagreement, ${result.synthesis.revision_instruction_count} revision instruction(s)` : 'missing'}`);
    console.log('');
...
function main() {
    ...
    const result = doctorWorkspaceReviewRun((0, verify_workspace_review_run_js_1.resolveRunDir)(args));
    if (args.json) {
