...
const { ROOT, parseArgs, requireArg, assertSafeCaseId, assertProbe, writeFileNew, slug, optionalSlugArg } = require('./lib');
function main() {
    ...
    const target = path.join(ROOT, 'runs', run, caseId, 'scores', ...(scoreVersion ? ['versions', scoreVersion] : []), `${slug(model)}-${probe}.score.json`);
