...
import { ROOT, agentOutputPaths, nodeScriptArgs, parseJsonFile, runtimeScript } from '../lib/lib.js'
...
function generatePrompts(run: string, probes: string[]): void {
  const result = runNode(runtimeScript('cli/generate-prompts'), [
    '--run',
    run,
    '--case',
...
function main() {
  ...
  const runModel = runtimeScript('cli/run-model')
  const runPool = runtimeScript('cli/run-agent-pool')
  const runCalibration = runtimeScript('cli/run-calibration')
  const runIds = [
...
`,
  ...
  try {
    ...
    let result = runNode(
      runModel,
      ['--run', retryRun, '--case', 'synthetic/event-reporting', '--model', 'kimi', '--probe', 'planner'],
      {
        ...baseEnv,
    ...
    result = runNode(
      runModel,
      ['--run', retryRun, '--case', 'synthetic/event-reporting', '--model', 'deepseek', '--probe', 'planner'],
      baseEnv,
    )
    ...
    result = runNode(
      runPool,
      ['--run', failureRun, '--cases', 'synthetic/event-reporting', '--models', 'deepseek', '--probes', 'planner'],
      {
        ...baseEnv,
    ...
    result = runNode(
      runCalibration,
      ['--run', missingOutputRun, '--case', 'synthetic/event-reporting', '--models', 'deepseek', '--probes', 'planner'],
      {
        ...baseEnv,
