...
import { ROOT, nodeScriptArgs, runtimeScript } from '../lib/lib.js'
import {
...
} from '../workspace/run-workspace-review.js'
...
import {
  ...
  progressSnapshot,
  getPlanReview,
} from '../mcp/plan-review-mcp.js'

type JsonRecord = Record<string, unknown>
type _RoleName = 'risk' | 'architecture' | 'execution' | 'rebuttal' | 'fact_check' | 'synthesis'
...
function retryExecutors(calls: RetryCalls) {
  return {
    runRole: async (config: WorkspaceConfigLike, _request: WorkspaceRequestLike, role: string, runDir: string) => {
...
async function main() {
  ...
  const server = runtimeScript('mcp/plan-review-mcp')
  try {
    ...
    const prompt = buildWorkspacePrompt('risk', tempDir, '检查当前计划', '只允许只读访问')
    ...
    const readScope = buildRoleReadScope(
      'risk',
      projectRoot,
      ['修改 `src/cli.ts` 和 `src/cdp/extract.ts`。', `不要读取 ${path.join(tempDir, 'outside.ts')}。`].join('\n'),
      {
        maxFiles: 10,
      },
    )
    ...
    const factScope = buildFactCheckReadScope(
      projectRoot,
      {
        'Risk Reviewer': {
          issues: [
    ...
    const factScopeWithConfigEvidence = buildFactCheckReadScope(
      projectRoot,
      {
        'Risk Reviewer': {
          issues: [
    ...
    const factScopeWithPlanRefs = buildFactCheckReadScope(
      projectRoot,
      {
        'Risk Reviewer': {
          issues: [
    ...
    const artifactScope = buildRoleReadScope('risk', projectRoot, compactedPlan.text, {
      maxFiles: 10,
      proposedArtifacts,
    })
    ...
    const refs = createPlanReferenceManifest(
      projectRoot,
      ['## Existing Code Refs', '- `src/cli.ts:1-1`', '- `/outside/project.ts:10`'].join('\n'),
      [],
    )
    ...
    assert.equal(args[args.indexOf('--allowed-tools') + 1], 'Read,Glob,Grep,mcp__json_validator__validate_json_output')
    ...
    assert(args[args.indexOf('--system-prompt') + 1].includes('mcp__json_validator__validate_json_output'))
    for (const [role, model, roleArgs, validatorLog] of [
      ...
      assert(validator.args.includes(runtimeScript('mcp/json-validator-mcp')))
    ...
    assert.doesNotThrow(() =>
      validateWorkspaceOutput('fact_check', {
        probe: 'fact_check',
        checked_issues: [],
        source_summaries: [],
        limits: [],
    ...
    assert.throws(
      () =>
        validateWorkspaceOutput('synthesis', {
          probe: 'synthesis',
          source_findings: [],
          process_map: {
            title: 'test',
    ...
    assert.doesNotThrow(() =>
      validateWorkspaceOutput('synthesis', validDiscardedSynthesis, { factCheckOutput: outOfScopeFactCheck }),
    )
    assert.throws(
      () =>
        validateWorkspaceOutput(
    ...
    assert.doesNotThrow(() =>
      validateWorkspaceOutput('synthesis', partiallyVerifiedRetainedSynthesis, {
        factCheckOutput: partiallyVerifiedFactCheck,
      }),
    )
    assert.throws(
    ...
    assert.throws(() => resolvePlanInput({}), /exactly one of plan or plan_file/)
    assert.throws(
      () =>
        resolvePlanInput({
          plan: '# 内联计划',
    ...
    const synthesisSchema = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'schemas', 'synthesis-output.schema.json'), 'utf8'),
    )
    const factCheckSchema = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'schemas', 'fact-check-output.schema.json'), 'utf8'),
    )
    assert.equal(factCheckSchema.properties.probe.const, 'fact_check')
    assert(factCheckSchema.properties.checked_issues.items.required.includes('issue_id'))
    assert(factCheckSchema.properties.checked_issues.items.required.includes('scope_status'))
    assert(synthesisSchema.required.includes('process_map'))
    assert(synthesisSchema.required.includes('source_findings'))
    assert(synthesisSchema.properties.consensus_issues.items.required.includes('affected_nodes'))
    assert(synthesisSchema.properties.consensus_issues.items.required.includes('source_finding_ids'))
    const executionSchema = JSON.parse(
      fs.readFileSync(path.join(ROOT, 'schemas', 'execution-output.schema.json'), 'utf8'),
    )
    assert(executionSchema.required.includes('coverage_declaration'))
    assert(
      executionSchema.properties.coverage_declaration.properties.reviewed_boundaries.items.properties.boundary.enum.includes(
        'implementation_discretion',
      ),
    )
    assert.equal(
    ...
    assert.doesNotThrow(() =>
      validateWorkspaceOutput('execution', {
        probe: 'execution',
    ...
    assert(
      synthesisPrompt.includes(
        'likely_false_positives` 只能引用 `disposition` 为 `duplicate`、`unsupported`、`contradicted`、`unverifiable` 或 `out_of_scope` 的 finding',
      ),
    )
    ...
    const emptyRefsScope = buildRoleReadScope(
      'risk',
      tempDir,
      '## Existing Code Refs\nNone\n\n## Tasks\nDo something.',
      { maxFiles: 10 },
    ...
    assert.equal(fs.readdirSync(path.join(reviewerRetry.runDir, 'roles', 'architecture-attempts')).length, 1)
    assert.equal(fs.readdirSync(path.join(reviewerRetry.runDir, 'roles', 'fact_check-attempts')).length, 1)
    assert.equal(fs.readdirSync(path.join(reviewerRetry.runDir, 'roles', 'synthesis-attempts')).length, 1)
    assert.equal(JSON.parse(fs.readFileSync(path.join(reviewerRetry.runDir, 'state.json'), 'utf8')).status, 'completed')
    const reviewerRetryManifest = JSON.parse(
      fs.readFileSync(path.join(reviewerRetry.runDir, 'run-manifest.json'), 'utf8'),
    )
    ...
    assert.match(architectureHistory[0].metadata_file, /^roles\/architecture-attempts\/[^/]+\/metadata\.json$/)
    assert.equal(architectureHistory[1].metadata_file, 'roles/architecture/metadata.json')
    ...
    const retryProgress = progressSnapshot(progressRetry.config, progressRetry.runDir, {
      status: 'queued',
      roles: ['risk', 'architecture'],
    })
