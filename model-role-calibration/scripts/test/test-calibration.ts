...
import { evaluationSchemaFile, validateEvaluationScore } from '../cli/evaluation-lib.js'
import { ROOT, loadConfig, loadCaseInput, schemaForProbe } from '../lib/lib.js'
import { DEFAULT_CASE } from '../cli/run-calibration.js'
import { MAX_CONCURRENCY, mergeRequestedJobs } from '../cli/run-agent-pool.js'
import { buildCliArgs, parseAssistantOutput, parseJsonEnvelope } from '../cli/run-model.js'
import { roleRecommendation } from '../cli/summarize-results.js'
...
function main() {
  ...
  const navigationStackCase = 'synthetic/navigation-stack-return'
  const navigationStackPlanner = loadCaseInput(navigationStackCase, 'planner')
  const navigationStackReview = loadCaseInput(navigationStackCase, 'execution')
  const navigationStackSynthesis = loadCaseInput(navigationStackCase, 'synthesis')
  const riskProbePrompt = fs.readFileSync(path.join(ROOT, 'prompts', 'probe-risk.md'), 'utf8')
  const factCheckProbePrompt = fs.readFileSync(path.join(ROOT, 'prompts', 'probe-fact_check.md'), 'utf8')
  const synthesisProbePrompt = fs.readFileSync(path.join(ROOT, 'prompts', 'probe-synthesis.md'), 'utf8')
  const synthesisEvaluatorPrompt = fs.readFileSync(path.join(ROOT, 'prompts', 'evaluate-synthesis.md'), 'utf8')
  assert(!config.primary_cases.includes(navigationStackCase))
  assert(navigationStackPlanner.includes('授权页内部只有统一返回动作'))
  assert(navigationStackReview.includes('replace(Authorization)'))
  assert(navigationStackReview.includes('push(Authorization)'))
  assert(navigationStackSynthesis.includes('返回效果可由导航栈自然产生'))
  assert(navigationStackSynthesis.includes('缺少字段不构成计划执行缺口'))
  assert(riskProbePrompt.includes('不得仅因没有 source、entry 或 route param 判定为风险'))
  assert(executionProbePrompt.includes('不得仅因没有 source、entry 或 route param 判定为执行缺口'))
  assert(executionProbePrompt.includes('禁止用 deeplink、通知、中间页等输入未声明入口来推导返回行为风险'))
  assert(factCheckProbePrompt.includes('若只是实现细节，或可由已声明控制流、导航栈、默认值或现有契约自然推出'))

  const trackedTodoCase = 'synthetic/tracked-todo-placeholder'
  const trackedTodoPlanner = loadCaseInput(trackedTodoCase, 'planner')
  const trackedTodoReview = loadCaseInput(trackedTodoCase, 'execution')
  const trackedTodoSynthesis = loadCaseInput(trackedTodoCase, 'synthesis')
  assert(!config.primary_cases.includes(trackedTodoCase))
  assert(trackedTodoPlanner.includes('代码中需要标注 TODO'))
  assert(trackedTodoReview.includes('关闭标准：后端确认正式枚举值后，替换代码中的 `SmsType.RFC = 21` 占位并关闭 TODO'))
  assert(trackedTodoSynthesis.includes('删除 TODO 的建议与需求目标冲突'))
  assert(trackedTodoSynthesis.includes('同一事项已在 Blocking Decisions 中跟踪'))
  assert(synthesisProbePrompt.includes('consensus_issues` 必须有实质性 `required_plan_change`'))
  assert(
    synthesisProbePrompt.includes('只能留在 `source_findings`，不得进入 `consensus_issues` 或 `revision_instructions`'),
  )
  assert(synthesisEvaluatorPrompt.includes('空字符串、无、无需新增修订或无需修改计划都表示该项不应进入 consensus'))
