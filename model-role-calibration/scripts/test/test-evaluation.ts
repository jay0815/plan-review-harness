...
import { ROOT, ensureDir, nodeScriptArgs, parseJsonFile, runtimeScript, writeGenerated } from '../lib/lib.js'
import {
  buildCodexArgs,
...
function main() {
  ...
  const runEvaluation = runtimeScript('cli/run-evaluation')
  const promoteEvaluation = runtimeScript('cli/promote-evaluation')
  const scoreOutput = runtimeScript('cli/score-output')
  const summarizeResults = runtimeScript('cli/summarize-results')
