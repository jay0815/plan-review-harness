...
function acquireRunLock(lockFile) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      let existing;
      try {
        existing = parseJsonFile(lockFile);
      } catch {
        existing = null;
      }
      if (!existing?.pid) {
...
function loadPoolIndex(indexFile) {
  ...
  const existing = parseJsonFile(indexFile);
...
function main() {
  ...
  const lockFile = process.env.MODEL_ROLE_CALIBRATION_POOL_LOCK
    || path.join(ROOT, "runs", ".agent-pool.lock");
