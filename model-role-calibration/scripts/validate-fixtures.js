...
function validateSynthesisFixture(caseId, input) {
    ...
    for (const source of new Set(expectedIssues.map((item) => item.source))) {
        ...
        const statusCounts = Object.fromEntries(['verified', 'partially_verified', 'unsupported', 'contradicted', 'unverifiable'].map((status) => [
            status,
            sourceIssues.filter((item) => item.status === status).length,
        ]));
        if (summary.total_issues !== sourceIssues.length ||
...
function main() {
    ...
    if (typeof config.role_recommendation.maximum_standard_deviation !== 'number' ||
    ...
    if (config.probe_concurrency_overrides !== undefined &&
