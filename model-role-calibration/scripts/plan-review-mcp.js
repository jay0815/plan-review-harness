...
exports.resolvePlanInput = resolvePlanInput;
exports.startPlanReview = startPlanReview;
exports.retryPlanReviewStage = retryPlanReviewStage;
exports.parseExecutionLogLine = parseExecutionLogLine;
exports.progressSnapshot = progressSnapshot;
exports.planReviewResult = planReviewResult;
...
function uniqueRunId(config, date = new Date()) {
    ...
    let runId = base;
    let index = 2;
    while (fs.existsSync((0, workspace_review_lib_js_1.runDirectory)(config, runId))) {
        runId = `${base}-${index}`;
        index += 1;
    }
    return runId;
    ...
    return [
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    roles: {
                        ...
                        items: {
                            type: 'string',
                            enum: workspace_review_lib_js_1.REVIEW_ROLES,
                        },
        ...
        },
        {
            ...
            description: [
                ...
                `每个 executor 最多重试 ${workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES} 次，超过上限会在调用模型前拒绝。`,
                '返回后必须立即使用 next_action 指定的参数调用 get_plan_review。',
            ].join(' '),
            annotations: {
                title: '重试计划评审阶段',
...
function resolvePlanInput(input) {
    ...
    if (stat.size > MAX_PLAN_BYTES) {
        throw new Error(`Plan file exceeds ${MAX_PLAN_BYTES} bytes: ${planFile}`);
    }
...
function startPlanReview(config, input) {
    ...
    const projectRoot = (0, workspace_review_lib_js_1.validateProjectRoot)(input.project_root || process.env.CLAUDE_PROJECT_DIR);
    const roles = input.roles?.length ? [...new Set(input.roles)] : workspace_review_lib_js_1.REVIEW_ROLES;
    ...
    const runDir = (0, workspace_review_lib_js_1.runDirectory)(config, runId);
    ...
    (0, lib_js_1.writeFileNew)(path.join(runDir, 'request.json'), JSON.stringify(request, null, 2) + '\n');
    (0, lib_js_1.writeFileNew)(path.join(runDir, 'state.json'), JSON.stringify({
        run_id: runId,
        status: 'queued',
        ...
        project_root: projectRoot,
        roles,
        execution_log: (0, workspace_review_lib_js_1.executionLogPath)(runDir),
        error: null,
    }, null, 2) + '\n');
    (0, workspace_review_manifest_js_1.createRunManifest)(config, request, runDir, {
        createdAt: request.created_at,
    });
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'run_queued', {
        run_id: runId,
        roles,
    });
    ...
    const runner = path.join(lib_js_1.ROOT, 'scripts', 'run-workspace-review.js');
    const child = (0, node_child_process_1.spawn)(process.execPath, [runner, ...config.loader_args, '--run-dir', runDir], {
    ...
    child.on('error', (error) => {
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'runner_start_failed', {
            run_id: runId,
        });
...
function retryPlanReviewStage(config, input) {
    ...
    const state = (0, lib_js_1.parseJsonFile)(stateFile);
    ...
    const roles = Array.isArray(state.roles) && state.roles.length ? state.roles : workspace_review_lib_js_1.REVIEW_ROLES;
    const reviewerCompleted = (role) => {
        ...
        return (0, lib_js_1.parseJsonFile)(metadataFile).status === 'completed';
    };
    ...
    if (input.stage === 'synthesis') {
        ...
        const factCheckMetadata = (0, lib_js_1.parseJsonFile)(path.join(runDir, 'roles', 'fact_check', 'metadata.json'));
    ...
    const plannedExecutors = input.stage === 'reviewers'
        ...
        : input.stage === 'fact_check'
            ...
            : ['synthesis'];
    const exhausted = [...new Set(plannedExecutors)].filter((executor) => Number(retryCounts[executor] || 0) >= workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES);
    if (exhausted.length) {
    ...
    (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'stage_retry_queued', {
        stage: input.stage,
        retry_roles: input.stage === 'reviewers' ? incompleteReviewers : [],
    });
    const stdoutFd = fs.openSync(path.join(runDir, 'runner.stdout.log'), 'a');
    const stderrFd = fs.openSync(path.join(runDir, 'runner.stderr.log'), 'a');
    const runner = path.join(lib_js_1.ROOT, 'scripts', 'retry-workspace-review-stage.js');
    const child = (0, node_child_process_1.spawn)(process.execPath, [runner, ...config.loader_args, '--run-dir', runDir, '--stage', input.stage, ...(input.force ? ['--force'] : [])], {
        cwd: path.resolve(lib_js_1.ROOT, '..'),
        detached: true,
        stdio: ['ignore', stdoutFd, stderrFd],
        env: (0, workspace_review_lib_js_1.withoutAnthropicApiKey)(process.env),
    ...
    child.on('error', (error) => {
        (0, workspace_review_lib_js_1.appendExecutionLog)(runDir, 'stage_retry_start_failed', {
            stage: input.stage,
        });
...
function progressSnapshot(config, runDir, state) {
    ...
    const logFile = (0, workspace_review_lib_js_1.executionLogPath)(runDir);
...
function planReviewResult(config, input) {
    const runDir = (0, workspace_review_lib_js_1.runDirectory)(config, input.run_id);
    const stateFile = path.join(runDir, 'state.json');
    if (!fs.existsSync(stateFile)) {
    ...
    if (includeReport && state.status === 'completed' && fs.existsSync(reportFile)) {
        result.report = (0, lib_js_1.parseJsonFile)(reportFile);
    }
...
async function getPlanReview(config, input, onProgress = null) {
    ...
    while (true) {
        ...
        const now = Date.now();
        if (onProgress && (signature !== lastSignature || now - lastNotificationAt >= PROGRESS_INTERVAL_MS)) {
            progressSequence += 1;
            onProgress({
    ...
    return async function handle(message, sendNotification = null) {
        if (message.method === 'initialize') {
            return {
                ...
                instructions: [
                    ...
                    `若 get_plan_review 返回失败，或 completed 但 Reviewer 未全部完成，可按 progress 调用 retry_plan_review_stage：reviewers 只重跑失败节点，fact_check 或 synthesis 复用已完成上游；每个 executor 最多重试 ${workspace_review_lib_js_1.MAX_EXECUTOR_RETRIES} 次。`,
        ...
        if (message.method === 'tools/call') {
            ...
            if (name === 'configuration_status') {
                return textResult({
                    valid: true,
                    ...(0, workspace_review_lib_js_1.configSummary)(config),
                    warnings: (0, workspace_review_lib_js_1.redactedSettingsWarnings)(config),
                });
            }
            ...
            if (name === 'get_plan_review') {
                ...
                const notifyProgress = sendNotification && (typeof progressToken === 'string' || typeof progressToken === 'number')
...
function main() {
    ...
    const warnings = (0, workspace_review_lib_js_1.redactedSettingsWarnings)(config);
    ...
    console.error(`[plan-review-mcp] configuration valid: ${Object.keys(config.models).join(', ')}`);
