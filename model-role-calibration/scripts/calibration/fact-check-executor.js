...
const { FACT_CHECK_ROOT, assertFactCheckCaseId, ingestOutput, loadCase, renderFactCheckPrompt, scoreOutput, summarizeRun: summarizeFactCheckRun, } = require('../fact-check-calibration-lib');
const { buildFactCheckReadScope, copyScopedWorkspace } = require('../workspace-review-lib');
void buildFactCheckReadScope;
void copyScopedWorkspace;
const { buildCliArgs, parseAssistantOutput, resolveWrapperCommand, runCommand } = require('../run-model');
...
function artifactPaths(run, caseId, model) {
    ...
    const modelSlug = (0, core_1.slug)(model);
    return {
        base,
...
function nextAttempt(paths) {
    (0, core_1.ensureDir)(paths.attemptsDir);
    const attempts = fs
...
function completedOutputExists(run, caseId, model) {
    const normalized = path.join(FACT_CHECK_ROOT, 'runs', run, caseId, 'outputs', 'normalized', `${(0, core_1.slug)(model)}.json`);
...
function writeCompletedArtifacts(paths, attempt, metadata, parsed) {
    ...
    metadata.error = null;
    (0, core_1.writeFileNew)(attempt.rawCliFile, JSON.stringify(parsed.envelope, null, 2) + '\n');
    (0, core_1.writeFileNew)(attempt.resultFile, JSON.stringify(parsed.output, null, 2) + '\n');
    (0, core_1.writeFileNew)(attempt.metadataFile, JSON.stringify(metadata, null, 2) + '\n');
    (0, core_1.writeGenerated)(paths.rawCliFile, JSON.stringify(parsed.envelope, null, 2) + '\n');
    (0, core_1.writeGenerated)(paths.resultFile, JSON.stringify(parsed.output, null, 2) + '\n');
    (0, core_1.writeGenerated)(paths.metadataFile, JSON.stringify(metadata, null, 2) + '\n');
}
class FactCheckExecutor {
    ...
    uniqueRunId(caseId) {
        const base = `${(0, core_1.slug)(caseId)}-${new Date()
            .toISOString()
            .replace(/[-:.]/g, '')
            .replace(/\d{3}Z$/, 'Z')}`;
        let run = base;
    ...
    generatePrompts({ run, caseId, models }) {
        ...
        (0, core_1.ensureDir)(promptDir);
        (0, core_1.writeGenerated)(path.join(promptDir, 'fact_check.md'), prompt);
        for (const model of models) {
            (0, core_1.writeGenerated)(path.join(promptDir, `${(0, core_1.slug)(model)}-fact_check.md`), prompt);
        }
        return {
            promptDir,
            ...
            prompts: models.map((model) => ({
                ...
                file: path.join(promptDir, `${(0, core_1.slug)(model)}-fact_check.md`),
            })),
        };
    }
    ...
    async runJob(job) {
        ...
        (0, core_1.ensureDir)(paths.outputDir);
        ...
        const config = (0, core_1.parseJsonFile)(path.join(path.dirname(FACT_CHECK_ROOT), 'calibration.config.json'));
        const executionConfig = config.agent_execution;
        const timeoutMs = (0, core_1.positiveInteger)(executionConfig.timeout_ms, 'agent_execution.timeout_ms');
        const maxBuffer = (0, core_1.positiveInteger)(executionConfig.max_buffer_bytes, 'agent_execution.max_buffer_bytes');
        ...
        const schema = (0, core_1.parseJsonFile)(schemaFile);
        const promptDir = promptPaths(run, caseId);
        const promptFile = path.join(promptDir, `${(0, core_1.slug)(model)}-fact_check.md`);
        const prompt = (0, core_1.readText)(promptFile);
        ...
        if (child.error || child.status !== 0) {
            ...
            if (child.stdout) {
                (0, core_1.writeFileNew)(attempt.rawTextFile, child.stdout);
            }
            (0, core_1.writeFileNew)(attempt.metadataFile, JSON.stringify(metadata, null, 2) + '\n');
