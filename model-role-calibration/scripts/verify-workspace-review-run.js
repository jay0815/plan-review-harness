...
const inspect_workspace_run_js_1 = require("./inspect-workspace-run.js");
...
function inspectIfReady(runDir) {
    ...
    return (0, inspect_workspace_run_js_1.inspect)(runDir);
}
...
function verifyRun(runDir) {
    ...
    if (manifest?.version === 1 && manifest.run_id === path.basename(absoluteRunDir)) {
        ...
        if (manifest.declared_runtime?.route_profile &&
    ...
    if (!factCheck) {
        ...
        const grantedNonReadTools = (factCheck.tools || []).filter((name) => name !== 'Read' && name !== JSON_VALIDATOR_TOOL);
    ...
    if (!synthesis) {
        ...
        const synthesisProjectToolCalls = Object.keys(synthesis.tool_counts || {}).filter((name) => name !== JSON_VALIDATOR_TOOL);
