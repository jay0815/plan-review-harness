...
exports.FACT_CHECK_ROOT = void 0;
exports.assertFactCheckCaseId = assertFactCheckCaseId;
exports.resolveWorkspaceRunDir = resolveWorkspaceRunDir;
exports.caseFile = caseFile;
exports.loadCase = loadCase;
exports.normalizedOutputFile = normalizedOutputFile;
exports.createCaseFromWorkspaceRun = createCaseFromWorkspaceRun;
exports.renderFactCheckPrompt = renderFactCheckPrompt;
exports.generatePrompts = generatePrompts;
exports.ingestOutput = ingestOutput;
exports.scoreOutput = scoreOutput;
exports.summarizeRun = summarizeRun;
const fs = __importStar(require("node:fs"));
const os = __importStar(require("node:os"));
const path = __importStar(require("node:path"));
const lib_js_1 = require("./lib.js");
exports.FACT_CHECK_ROOT = process.env.MODEL_ROLE_CALIBRATION_FACT_CHECK_ROOT
    ? path.resolve(process.env.MODEL_ROLE_CALIBRATION_FACT_CHECK_ROOT)
    : path.join(lib_js_1.ROOT, 'fact-check-calibration');
const DEFAULT_WORKSPACE_RUNS_DIR = path.join(os.homedir(), '.claude', 'plan-review-harness', 'mcp', 'workspace-runs');
const REVIEWER_SOURCE_BY_ROLE = {
    risk: 'Risk Reviewer',
    architecture: 'Architecture Reviewer',
    execution: 'Execution Reviewer',
    rebuttal: 'Rebuttal Reviewer',
};
const SOURCE_ALIAS_BY_NORMALIZED = new Map(Object.entries({
    risk: 'Risk Reviewer',
...
function caseDir(caseId) {
    ...
    return path.join(exports.FACT_CHECK_ROOT, 'cases', caseId);
}
...
function runCaseDir(run, caseId) {
    assertFactCheckCaseId(caseId);
    return path.join(exports.FACT_CHECK_ROOT, 'runs', run, caseId);
}
function normalizedOutputFile(run, caseId, model) {
    return path.join(runCaseDir(run, caseId), 'outputs', 'normalized', `${(0, lib_js_1.slug)(model)}.json`);
}
function scoreFile(run, caseId, model) {
    return path.join(runCaseDir(run, caseId), 'scores', `${(0, lib_js_1.slug)(model)}.score.json`);
}
...
function checkedIssueKey(source, title) {
    return `${normalizeSource(source)}\u0000${title}`;
}
function reviewerOutputsFromReport(report) {
    return Object.fromEntries(Object.entries(report.reviewers || {}).map(([role, item]) => [
        REVIEWER_SOURCE_BY_ROLE[role] || role,
        item.output,
    ]));
}
function issueRowsFromReviewers(reviewerOutputs, factCheckOutput) {
    const factRowsById = new Map((factCheckOutput?.checked_issues || []).filter((item) => item.issue_id).map((item) => [item.issue_id, item]));
    const factRowsByText = new Map((factCheckOutput?.checked_issues || []).map((item) => [checkedIssueKey(item.source, item.issue_title), item]));
    const rows = [];
    for (const [source, output] of Object.entries(reviewerOutputs)) {
        const issues = Array.isArray(output?.issues) ? output.issues : [];
        issues.forEach((issue, index) => {
            const id = `${(0, lib_js_1.slug)(source)}-${String(index + 1).padStart(3, '0')}`;
...
function createCaseFromWorkspaceRun({ runId, runDir, caseId }) {
    assertFactCheckCaseId(caseId);
    ...
    const report = (0, lib_js_1.parseJsonFile)(reportFile);
    ...
    const fixture = {
        ...
        plan: {
            ...
            review_plan: fs.existsSync(reviewPlanFile) ? (0, lib_js_1.readText)(reviewPlanFile) : String(request.plan || ''),
        },
    ...
    (0, lib_js_1.writeFileNew)(target, JSON.stringify(fixture, null, 2) + '\n');
    return {
        case_file: target,
        issue_count: fixture.issues.length,
    };
}
function renderFactCheckInput(fixture) {
...
function renderFactCheckPrompt(fixture) {
    const template = (0, lib_js_1.readText)(path.join(lib_js_1.ROOT, 'prompts', 'probe-fact_check.md'));
    return template.replace('{{INPUT}}', renderFactCheckInput(fixture));
}
function generatePrompts({ run, caseId, models }) {
    ...
    for (const model of models) {
        (0, lib_js_1.writeGenerated)(path.join(promptDir, `${(0, lib_js_1.slug)(model)}-fact_check.md`), prompt);
    }
    return {
...
function ingestOutput({ run, caseId, model, file }) {
    ...
    const output = JSON.parse((0, lib_js_1.readText)(absoluteFile));
...
function validateFactCheckOutput(output) {
...
function textIssueKey(item) {
    return checkedIssueKey(item.source, item.issue_title);
}
function expectedIssues(fixture) {
...
function scoreOutput({ run, caseId, model }) {
    const fixture = loadCase(caseId);
    const expected = expectedIssues(fixture);
    const normalized = (0, lib_js_1.parseJsonFile)(normalizedOutputFile(run, caseId, model));
    const output = normalized.output;
    validateFactCheckOutput(output);
    const actualById = new Map(output.checked_issues.filter((item) => item.issue_id).map((item) => [item.issue_id, item]));
    const actualLegacyByText = new Map(output.checked_issues.filter((item) => !item.issue_id).map((item) => [textIssueKey(item), item]));
    ...
    const challengedHits = challengedExpected.filter((item) => item.actual_status !== 'verified' && item.actual_status !== 'missing');
    const falseVerified = challengedExpected.filter((item) => item.actual_status === 'verified');
    const overChallenged = rows.filter((item) => item.expected_status === 'verified' && !['verified', 'missing'].includes(item.actual_status));
    ...
    const score = {
        ...
        metrics: {
            ...
            evidence_status_accuracy: evidenceLabeled.length
                ? Number((evidenceLabeled.filter((item) => item.evidence_status_match).length / evidenceLabeled.length).toFixed(4))
                : null,
    ...
    (0, lib_js_1.writeGenerated)(target, JSON.stringify(score, null, 2) + '\n');
    return score;
}
function summarizeRun(run) {
    const runRoot = path.join(exports.FACT_CHECK_ROOT, 'runs', run);
    const scores = [];
        ...
        for (const caseEntry of fs.readdirSync(runRoot, { withFileTypes: true })) {
            ...
            for (const scoreEntry of fs.readdirSync(scoreDir, { withFileTypes: true })) {
                if (scoreEntry.isFile() && scoreEntry.name.endsWith('.score.json')) {
                    scores.push((0, lib_js_1.parseJsonFile)(path.join(scoreDir, scoreEntry.name)));
                }
            }
        }
    }
    ...
    const model_summaries = models
        ...
        return {
            ...
            avg_status_accuracy: average(rows.map((item) => item.metrics.status_accuracy).filter((item) => item !== null)),
            avg_challenge_recall: average(rows.map((item) => item.metrics.challenge_recall).filter((item) => item !== null)),
        ...
        .sort((a, b) => (b.avg_status_accuracy ?? -1) - (a.avg_status_accuracy ?? -1) ||
    ...
    (0, lib_js_1.writeGenerated)(path.join(exports.FACT_CHECK_ROOT, 'outputs', `${run}.summary.json`), JSON.stringify(summary, null, 2) + '\n');
    (0, lib_js_1.writeGenerated)(path.join(exports.FACT_CHECK_ROOT, 'outputs', `${run}.summary.md`), renderSummary(summary));
    return summary;
}
...
function renderSummary(summary) {
    ...
    lines.push('| Model | Cases | Status Accuracy | Challenge Recall | False Verified | Over Challenged | Extra | Missing |');
    lines.push('|---|---:|---:|---:|---:|---:|---:|---:|');
    for (const item of summary.model_summaries) {
        lines.push(`| ${item.model} | ${item.cases.length} | ${pct(item.avg_status_accuracy)} | ${pct(item.avg_challenge_recall)} | ${item.total_false_verified} | ${item.total_over_challenged} | ${item.total_extra} | ${item.total_missing} |`);
    }
