...
const FACT_CHECK_ROOT = process.env.MODEL_ROLE_CALIBRATION_FACT_CHECK_ROOT
...
function issueRowsFromReviewers(reviewerOutputs, factCheckOutput) {
  const factRowsById = new Map((factCheckOutput?.checked_issues || []).filter((item) => item.issue_id).map((item) => [
    item.issue_id,
    item
  ]));
  const factRowsByText = new Map((factCheckOutput?.checked_issues || []).map((item) => [
    checkedIssueKey(item.source, item.issue_title),
    item
  ]));
  const rows = [];
...
function createCaseFromWorkspaceRun({ runId, runDir, caseId }) {
  ...
  const report = parseJsonFile(reportFile);
  ...
  const fixture = {
    ...
    plan: {
      ...
      review_plan: fs.existsSync(reviewPlanFile)
        ? readText(reviewPlanFile)
        : String(request.plan || "")
    },
...
function renderFactCheckInput(fixture) {
  ...
  const sourceSections = Object.entries(fixture.reviewer_outputs || {}).map(([source, output]) => [
    `## ${source}`,
    "",
    "```json",
...
function scoreOutput({ run, caseId, model }) {
  ...
  const normalized = parseJsonFile(normalizedOutputFile(run, caseId, model));
  const output = normalized.output;
  validateFactCheckOutput(output);
  const actualById = new Map(output.checked_issues.filter((item) => item.issue_id).map((item) => [
    item.issue_id,
    item
  ]));
  const actualLegacyByText = new Map(output.checked_issues.filter((item) => !item.issue_id).map((item) => [
    textIssueKey(item),
    item
  ]));
  ...
  const rows = expected.map((item) => {
    ...
    return {
      ...
      claim_support_match: item.expected_claim_support
        ? actual?.claim_support === item.expected_claim_support
        : null,
    ...
    for (const caseEntry of fs.readdirSync(runRoot, { withFileTypes: true })) {
      ...
      for (const scoreEntry of fs.readdirSync(scoreDir, { withFileTypes: true })) {
        if (scoreEntry.isFile() && scoreEntry.name.endsWith(".score.json")) {
          scores.push(parseJsonFile(path.join(scoreDir, scoreEntry.name)));
        }
      }
    }
  }
  const models = [...new Set(scores.map((item) => item.model))].sort();
  const model_summaries = models.map((model) => {
