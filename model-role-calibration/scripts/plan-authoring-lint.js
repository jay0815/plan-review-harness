...
exports.LINE_BUDGETS = void 0;
exports.parseSections = parseSections;
exports.parseExistingCodeRefs = parseExistingCodeRefs;
exports.parseCodeBlocks = parseCodeBlocks;
exports.classifyCodeBlock = classifyCodeBlock;
...
exports.LINE_BUDGETS = LINE_BUDGETS;
const REQUIRED_SECTION_GROUPS = [
    ...
    { name: 'existing_code_refs', patterns: [workspace_review_lib_js_1.EXISTING_CODE_REFS_HEADING_PATTERN] },
...
function parseExplicitComplexity(plan, sections) {
    const jsonMatch = String(plan).match(/"plan_complexity"\s*:\s*\{[\s\S]{0,300}?"level"\s*:\s*"(single_file|feature|cross_feature|architecture)"/i);
    if (jsonMatch) {
        return {
    ...
    const inlineMatch = String(plan).match(/(?:plan[_ ]complexity|complexity|计划复杂度)\s*[:：]\s*(single_file|feature|cross_feature|architecture)\b/i);
    if (inlineMatch) {
        return {
...
function existingCodeRefsSections(sections) {
    ...
    for (let index = 0; index < sections.length; index += 1) {
        ...
        if (!workspace_review_lib_js_1.EXISTING_CODE_REFS_HEADING_PATTERN.test(section.title)) {
            continue;
        }
...
function collectCandidateLineNumbers(sections) {
    ...
    for (const section of existingCodeRefsSections(sections)) {
        for (const entry of section.lines) {
            ...
            const candidates = pathMatch ? [pathMatch[1].trim().replace(/^`|`$/g, '')] : (0, workspace_review_lib_js_1.collectPathCandidates)(entry.text);
...
function parseExistingCodeRefs(sections, projectRoot = process.cwd()) {
    ...
    const manifest = (0, workspace_review_lib_js_1.createPlanReferenceManifest)(projectRoot, sectionsToPlan(sections), []);
    const inlineRefs = [
    ...
    ].filter((ref) => !structuredKeys.has(manifestRefKey(ref)) &&
...
function precedingHeading(sections, line) {
    return (sections.filter((section) => section.start_line <= line).sort((a, b) => b.start_line - a.start_line)[0]?.title || '');
}
...
function classifyCodeBlock(block, sectionTitle) {
    ...
    const nonEmpty = code
        ...
        .map((line) => line.trim())
        .filter(Boolean);
    const looksLikeTree = lines <= 20 &&
        nonEmpty.length > 0 &&
        nonEmpty.every((line) => /^[│├└─\s]+[A-Za-z0-9_.@/-]+$/.test(line) ||
    ...
    const interfaceOnly = lines <= 10 && /\b(interface|type)\b/.test(code) && !/\bfunction\b|=>|\breturn\b|<[A-Z][A-Za-z0-9]*/.test(code);
    if (interfaceOnly) {
        return {
    ...
    const hasTestImplementation = /\b(describe|test|it)\s*\(|\bexpect\s*\(/.test(code) &&
    ...
    const hasHookOrComponent = /\bfunction\s+use[A-Z]\w*\s*\(|\bconst\s+use[A-Z]\w*\s*=/.test(code) ||
    ...
    const hasCompleteFunction = (/\b(?:async\s+)?function\s+\w*\s*\([^)]*\)\s*(?::\s*\w[^\s{]*)?\s*\{/.test(code) ||
...
function validateExistingCodeRefs(refs, projectRoot, errors) {
    ...
    for (const ref of refs.filter((item) => item.source !== 'inline')) {
        if (!ref.path || !ref.lines || !ref.symbol || !ref.reason) {
            errors.push(issue('existing_ref_incomplete', 'Existing Code Ref 必须包含 path、lines、symbol 和 reason', {
                line: ref.path_line,
                path: ref.path || null,
            }));
            continue;
        ...
        if (path.isAbsolute(ref.path) || ref.path.split(/[\\/]/).includes('..')) {
            errors.push(issue('existing_ref_outside_project', 'Existing Code Ref 必须是 project root 内的相对路径', {
                line: ref.path_line,
                path: ref.path,
            }));
            continue;
        ...
        if (/^proposed-code\//.test(ref.path) || /(?:^|\/)(?:future|proposed|draft)(?:-code)?(?:\/|$)/i.test(ref.path)) {
            errors.push(issue('existing_ref_future_path', '未来文件或 proposed-code 不能作为 Existing Code Ref', {
                line: ref.path_line,
                path: ref.path,
            }));
            continue;
        ...
        if (relative.startsWith('..') || path.isAbsolute(relative)) {
            errors.push(issue('existing_ref_outside_project', 'Existing Code Ref 超出 project root', {
                line: ref.path_line,
                path: ref.path,
            }));
            continue;
        ...
        if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile()) {
            errors.push(issue('existing_ref_file_missing', 'Existing Code Ref 文件不存在', { line: ref.path_line, path: ref.path }));
            continue;
        }
        const lineMatch = ref.lines.match(/^(\d+)(?:\s*-\s*(\d+))?$/);
        if (!lineMatch) {
            errors.push(issue('existing_ref_lines_invalid', 'Existing Code Ref lines 必须是 N 或 N-M', {
                line: ref.path_line,
                path: ref.path,
                lines: ref.lines,
            }));
        ...
        if (start < 1 || end < start || end > fileLines.length) {
            errors.push(issue('existing_ref_lines_out_of_range', 'Existing Code Ref 行号超出文件范围', {
                line: ref.path_line,
                path: ref.path,
                lines: ref.lines,
        ...
        if (!excerpt.includes(ref.symbol)) {
            errors.push(issue('existing_ref_symbol_missing', 'Existing Code Ref 的 symbol 不在指定行号范围内', {
                line: ref.path_line,
                path: ref.path,
                lines: ref.lines,
                symbol: ref.symbol,
...
function lintPlan({ plan, projectRoot }) {
    ...
    const codeBlockMetrics = codeBlocks.map((block) => {
        ...
        if (!classification.allowed && classification.reliable) {
            ...
            errors.push(issue('implementation_code_block', `Plan 包含可可靠识别的完整实现型代码块：${classification.kind}`, {
                block: block.index,
            ...
            warnings.push(issue('large_unclassified_code_block', '存在较长代码块，无法可靠判断是否为实现草案；请确认其只表达必要契约', {
                block: block.index,
                line: block.start_line,
                language: block.language || null,
                line_count: block.line_count,
    ...
    if (codeBlocks.length > maxCodeBlocks) {
        errors.push(issue('code_block_budget_exceeded', '代码块数量超过每 100 行最多两个的预算', {
    ...
    if (lineBudgetExceeded) {
        warnings.push(issue('line_budget_exceeded', `计划行数超过 ${complexity.level} 预算`, {
            actual: totalLines,
            budget: lineBudget,
            complexity: complexity.level,
        }));
        if (implementationCodeBlocks > 0) {
            errors.push(issue('line_budget_with_implementation_bloat', '计划同时超过行数预算并包含完整实现型代码块', {
    ...
    for (const section of sections) {
        ...
        for (const line of section.lines) {
            ...
            for (const match of matches) {
                warnings.push(issue('uncertain_wording_outside_decision_section', `待确认措辞“${match[0]}”不在允许章节中`, {
                    line: line.number,
...
function main() {
    ...
    const planFile = path.resolve((0, lib_js_1.requireArg)(args, 'plan'));
    const projectRoot = path.resolve((0, lib_js_1.requireArg)(args, 'project-root'));
