...
exports.requireRunManifest = requireRunManifest;
exports.createRunManifest = createRunManifest;
exports.updateRunManifest = updateRunManifest;
exports.markManifestRunning = markManifestRunning;
exports.markManifestFinished = markManifestFinished;
exports.recordResolvedExecution = recordResolvedExecution;
exports.archiveResolvedExecutionAttempt = archiveResolvedExecutionAttempt;
exports.backfillRunManifest = backfillRunManifest;
exports.hashText = hashText;
exports.hashJson = hashJson;
exports.hashFileIfExists = hashFileIfExists;
...
const DEFAULT_ROUTE_FILE = path.join(lib_js_1.ROOT, 'default-role-routes.json');
...
function workspacePromptFile(role) {
    return path.join(lib_js_1.ROOT, 'prompts', `probe-${role}.md`);
}
function workspaceSchemaFile(role) {
    if (role === 'fact_check') {
        return path.join(lib_js_1.ROOT, 'schemas', 'fact-check-output.schema.json');
    }
    return path.join(lib_js_1.ROOT, 'schemas', `${role}-output.schema.json`);
}
function hashFilesByRole(roles, resolver) {
    ...
    for (const role of roles) {
        ...
        items[role] = {
            path: path.relative(lib_js_1.ROOT, file),
...
function declaredRuntime(config, roles) {
    const runtimeRoles = [...new Set([...roles, 'fact_check', 'synthesis'])].filter((role) => WORKSPACE_REVIEW_ROLES.includes(role));
    ...
    const routeProfile = {
        path: path.relative(lib_js_1.ROOT, routePath),
...
function schemaHash(metadata) {
    const schemaFile = metadata.schema_file ? path.join(lib_js_1.ROOT, metadata.schema_file) : workspaceSchemaFile(metadata.role);
