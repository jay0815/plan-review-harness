...
1. 校准 Architecture Reviewer，确认是否允许输出结构性修订目标。
2. 为 Architecture 建立独立 Schema 和评估器。
3. 校准 Execution Reviewer，移除对不存在文件和命令的诱导。
4. 为 Execution 建立独立 Schema 和评估器。
5. 校准 Rebuttal Reviewer，明确与 Risk 的差异并决定是否移除建议。
6. 为 Rebuttal 建立独立 Schema 和评估器。
7. 校准 Synthesizer 的来源保真、分歧分类和裁决边界。
8. 完成 Synthesis 评估器。
9. 回看 Planner 初始化 prompt，加入权威契约和阻塞决策字段。
10. 使用已确认评分样本测试 Planner 与 Risk 评估器。
11. 所有评估器稳定后，再接入 Codex CLI 自动评分。
