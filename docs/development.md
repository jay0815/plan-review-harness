...
`model-role-calibration/scripts/**/*.ts` 是校准工具链源码，package scripts 通过 `node --import tsx` 直接执行。只有修改 `model-role-calibration/` 或相关 package scripts 时，才需要把它纳入本轮验证。

`model-role-calibration/package.json` 只用于声明 CommonJS 模块边界，避免根包 `"type": "module"` 改变脚本中 `__dirname`、`__filename` 等运行语义。`model-role-calibration/tsconfig.json` 纳入 `.ts` 和 `.d.ts`；修改 TS 源后运行 `pnpm calibration:typecheck` 和相关测试。
