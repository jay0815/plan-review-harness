...
`model-role-calibration/` 是历史 CommonJS 校准工具链，使用子目录 `package.json` 与根包的 `"type": "module"` 隔离。默认 `pnpm test` 只运行核心 Vitest 测试；历史 JS 脚本通过 `pnpm calibration:test` 单独验证，后续迁移出的 TS 文件通过 `pnpm calibration:typecheck` 检查，并由 `pnpm calibration:build` 生成兼容 JS。
