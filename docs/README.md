...
本仓库是单包 TypeScript ESM 项目。CLI 当前只暴露 `start`；`LangGraphWorkflowRuntime` 中已有 resume 行为，但尚未提供 CLI resume 命令。当前可运行路径使用 `MockAgentWorkerAdapter` 和 `fixtures/mock/` 下的 fixture；其他非 mock worker kind 仍是类型层面的预留能力，尚未实现适配器。
