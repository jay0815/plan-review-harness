...
`model-role-calibration/` 是校准工具链，和根 TypeScript ESM harness 分开维护。源码位于 `scripts/**/*.ts`，由 `pnpm calibration:typecheck` 检查，并由 `pnpm calibration:build` 生成 CommonJS 兼容 JS。
